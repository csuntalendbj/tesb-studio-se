...
public class RouteResourceEditorUtil {
    ...
    /**
     * find a prefer editor from all existing editors according to the fileExtensions
     * 
     * @param fileExtension
     * @return
     */
     ...
     */
    public static void openEditor(final IWorkbenchPage page, IRepositoryNode node, RouteResourceItem item) {
        RouteResourceInput fileEditorInput = RouteResourceInput.createInput(node, item);
        if (fileEditorInput.isReadOnly()) {
            openDefaultEditor(page, node, item);
        } else {
            openEditor(page, fileEditorInput, item, RouteResourceEditorUtil.INSTANCE.findPreferEditor(fileEditorInput));
        }
    }
     ...
     */
    public static void openEditor(final IWorkbenchPage page, RouteResourceInput fileEditorInput, RouteResourceItem item,
            String editorId) {
        try {
            IEditorPart editorPart = page.findEditor(fileEditorInput);

            page.getWorkbenchWindow().getPartService().addPartListener(new ResourceEditorListener(fileEditorInput, page));

            if (!RouteResourceEditor.ID.endsWith(editorId)) {
                ResourcesPlugin.getWorkspace().addResourceChangeListener(new RouteResoureChangeListener(fileEditorInput));
            }
    ...
    private static boolean isLatestVersion(Property property) {
        try {
            List<IRepositoryViewObject> allVersion = CoreRuntimePlugin.getInstance().getProxyRepositoryFactory()
