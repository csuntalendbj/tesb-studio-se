...
/**
 * The Action binding to a Talend Component UI eidotr, and only effect to cTalendJob component. Use to support a simple
 * access point for editor job that linked to a cTalendJob component.
 */
public class OpenTalendJobRefAction extends org.talend.core.ui.editor.CustomExternalActions {
    ...
    /*
     * Will send a "open" request to the selected nodePart, so it will try to open the reference job. If the "open"
     * command success, then will try to switch the perspective to Integration.
     */
    @Override
    public void run() {
        ...
        try {
            ...
            PlatformUI.getWorkbench().showPerspective(ORG_TALEND_RCP_INTEGRATION_PERSPECTIVE,
