...
/**
 * Use to generate connection prefix or suffix for some special component. For example, component cLoop's loop flow is
 * wrapped in a subProcess, need to append an ".end()" after the whole loop process(before route connecton). Normally,
 * the ".end()" tag will append before the Route connection(ROUTE_ENDBLCOK type). But when no Route conneciton, it will
 * parse wrong. This class is used in cases like this to provide connection prefix/suffix. </br>
